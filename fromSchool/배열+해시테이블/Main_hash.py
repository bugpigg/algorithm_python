...
#          5. 또한 for문을 돌때 for문 변수가 가리키는 해시테이블의 아이템도 삭제하여야 예외처리가 가능하다.
#          6. 5.번의 step은 다음과 같은 상황을 방지한다. e.g.) k =10, n개의 정수 =[5,2,8] 일때 5.번 step이 없다면 정답 1이 아닌 2가 출력된다.
# 수행시간: 위 알고리즘은 해시테이블을 사용하여 삽입, 삭제, 검색 연산에서 O(1)시간이 소요된다. 
#          그러므로 for문에서 n개의 정수를 탐색하는 시간만 카운트하면 되므로 O(n)의 수행시간이 소요된다.
# 수행시간 분석: 1. 해시테이블 사용: O(n)의 수행시간
#               2. 정렬 사용: O(nlogn)의 수행시간 
#                  ㄴ 알고리즘: 1. 배열 정렬 후 front_pointer, back_pointer 두 개 선언
#                              2. 배열의 맨 앞과 맨 뒤의 숫자를 두 포인터로 가리키면서 두 숫자를 더해서 비교
#                              3. if 두 숫자의 합이 target 값보다 크다면 -> back_pointer 1 감소
#                                 elif 두 숫자의 합이 target 값보다 작다면 -> front_pointer 1 증가
#                                 else 두 숫자의 합이 target과 같다면 -> count 증가
#                              4. 위 의 방식으로 전체 count 리턴
...
#               4. n을 키워가며 비교했을 때 해시테이블 방법이 O(n)의 수행시간을 따르고 정렬방법이 O(nlogn)의 수행시간을 따르는것으로 보여지며, 해시테이블 방법이 더 빠른 수행시간을 보여준다.
def solve(target, arr):
    new_dict ={}
    count = 0
    for idx,num in enumerate(arr):# 해시테이블에 key,value 값 저장
        new_dict[num] = idx
    for num in arr:
